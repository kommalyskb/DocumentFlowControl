...
namespace DFM.Frontend.Pages
{
    public partial class DynamicFlow
    {
            ...
            try
            {
                ...
                if (isDelete.HasValue)
                {
                    if (isDelete.Value)
                    {
                        ...
                        //string url = $"{endpoint.API}/api/v1/RuleMenu/RemoveItem/";
        ...
        void onRowClick(DynamicItem item)
        {
            // Row click
        ...
        async Task onSaveClickAsync()
        {
            try
            {
                ...
                string url = $"{endpoint.API}/api/v1/Organization/SaveDynamicFlow/{employee.OrganizationID}";
                var result = await httpService.Post<DynamicItem, CommonResponse>(url, dynamicItem, new AuthorizeHeader("bearer", token), cancellationToken: cts.Token);
